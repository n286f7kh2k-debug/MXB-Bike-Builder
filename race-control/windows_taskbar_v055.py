...
from .windows_integration import APP_ID,best_icon,find_shortcut,ensure_desktop_shortcut
...
def _load_from_spec(spec,large=True):
    ...
    try:
        import ctypes
        from ctypes import wintypes
        user32=ctypes.windll.user32;suffix=Path(path).suffix.lower();h=0
        if suffix=='.ico':
            IMAGE_ICON=1;LR_LOADFROMFILE=0x0010|0x0040
            fn=user32.LoadImageW;fn.restype=wintypes.HANDLE
            h=fn(None,path,IMAGE_ICON,48 if large else 16,48 if large else 16,LR_LOADFROMFILE)
        else:
            big=wintypes.HICON();small=wintypes.HICON();fn=ctypes.windll.shell32.ExtractIconExW
...
def apply_taskbar_identity(tk_root,icon_path='',app_id=APP_ID):
    ...
    try:
        ...
        try:ensure_desktop_shortcut(app_root)
        except Exception:pass
        # Explorer's rendered shortcut icon is the first choice. That guarantees the running
        # taskbar button matches the desktop logo even when IconLocation is an EXE/DLL resource.
        shortcut=find_shortcut(refresh=True);spec=best_icon(app_root,refresh=True) or icon_path
        big=_load_from_shortcut(shortcut,True) or _load_from_spec(spec,True)
        small=_load_from_shortcut(shortcut,False) or _load_from_spec(spec,False) or big
        if not big:return False
        user32=ctypes.windll.user32;send=user32.SendMessageW;send.restype=wintypes.LRESULT
        WM_SETICON=0x0080;ICON_SMALL=0;ICON_BIG=1;GCLP_HICON=-14;GCLP_HICONSM=-34
        SWP_NOMOVE=0x0002;SWP_NOSIZE=0x0001;SWP_NOZORDER=0x0004;SWP_NOACTIVATE=0x0010;SWP_FRAMECHANGED=0x0020
        ...
        for hwnd in _top_hwnds(tk_root):
            ...
            try:user32.SetWindowPos(hwnd,None,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE|SWP_NOZORDER|SWP_NOACTIVATE|SWP_FRAMECHANGED)
            except Exception:pass
            try:user32.RedrawWindow(hwnd,None,None,0x0001|0x0100|0x0400)
            except Exception:pass
            try:ctypes.windll.dwmapi.DwmInvalidateIconicBitmaps(hwnd)
            except Exception:pass
        try:
            SHCNE_ASSOCCHANGED=0x08000000;SHCNF_IDLIST=0
            ctypes.windll.shell32.SHChangeNotify(SHCNE_ASSOCCHANGED,SHCNF_IDLIST,None,None)
        except Exception:pass
        try:tk_root._taskbar_icon_source=str(shortcut or spec)
        except Exception:pass
        return True
    except Exception:return False
