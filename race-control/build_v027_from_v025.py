...
NOTES = ('MXB Race Day Live v0.2.7: fixes automatic restart after an in-app update, preserves the GitHub API update check, '
...
work = Path(tempfile.mkdtemp(prefix='mxbv027v025_'))
with zipfile.ZipFile(BASE) as z:
    z.extractall(work)

p = work / 'src/config.py'
s = p.read_text(encoding='utf-8')
assert "VERSION = '0.2.5'" in s
p.write_text(s.replace("VERSION = '0.2.5'", "VERSION = '0.2.7'"), encoding='utf-8')
...
old_check = '''def check_for_update():
    ...
    try:
        ...
        raise UpdateError(f'Could not contact the update server: {exc}') from exc
