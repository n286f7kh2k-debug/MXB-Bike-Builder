...
NOTES=('MXB Race Day Live v0.3.8: fixes the Windows taskbar icon by taking the exact icon Windows already displays for the MXB Race Day Live desktop shortcut and applying it directly to the live app HWND. '
       'A fresh AppUserModelID bypasses stale Python/Tk shell caching; WM_SETICON sets both large and small taskbar icons, the top-level class icons are updated, and the icon is re-applied after window mapping so Tk/Windows cannot replace it during startup. '
       'If the shortcut cannot be found, Race Day Live falls back to any local packaged ICO it can locate. Existing Garage, performance optimizations, updater/restart, one-click joining, memberships, wallet, race economics, track artwork, results and admin controls are preserved.')
...
start=app.index(anchor); end=app.find('\n    def ',start+len(anchor)); end=len(app) if end<0 else end
close_block=app[start:end]
if 'release_taskbar_icons()' not in close_block:
    needle='        try:self.conn.close()\n        except Exception:pass\n'
    if needle in close_block:close_block=close_block.replace(needle,needle+"        try:release_taskbar_icons()\n        except Exception:pass\n",1)
    else:
        needle='        self.destroy()'
...
assert 'SHGetFileInfoW' in taskc and '*.lnk' in taskc and 'find_race_day_live_shortcut' in taskc
assert 'MXB Race Day Live.lnk' in taskc and 'LoadImageW' in taskc and 'RedrawWindow' in taskc and 'release_taskbar_icons' in appc
...
print('V038 VERIFIED',{'desktop_shortcut_exact_icon':True,'fresh_app_user_model_id':True,'wm_seticon_big_small':True,'class_icon_hwnd':True,'map_reapply':True,'local_ico_fallback':True,'v037_garage_preserved':True,'performance_preserved':True})
