...
for p in work.rglob('*.py'):
 ...
 for pat in ['def register_with_wallet','CREATE TABLE registrations','CREATE TABLE payment_transactions','def wallet_debit','def wallet_credit','def current_purse','def fastest_lap_pool','def race_financials']:
  print('\n---',pat,'---')
  for m in list(re.finditer(pat,s,re.I))[:6]:
   a=max(0,m.start()-500); b=min(len(s),m.end()+3000)
